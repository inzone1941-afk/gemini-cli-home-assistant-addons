...
This repository contains Home Assistant add-ons, specifically the **Gemini CLI Terminal** add-on which provides a web-based terminal interface with the Google **Gemini CLI** pre-installed. The add-on allows Home Assistant users to access Gemini AI capabilities directly from their dashboard.
